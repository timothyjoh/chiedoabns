	...
	$.wpProQuizFront = function(element, options) {
		...
		*/
		...
		var globalElements = {
			...
			wdmcheck: $e.find(globalNames.wdmcheck),
		...
		/*Question Check callback method
		 *Added by Bharat
		 *
		 *Called by wdmcheckQuestion method
		 *
		 *Returns response message & answer is correct or incorrect
		 *
		 *Date: 28 Nov, 2014
		 */
		var wdmchecker = function(name, data, $question, $questionList) {
			var correct = true;
			var response = '';
			...
			var func = {
				singleMulti: function() {
					...
					$questionList.children().each(function(i) {
						...
						if ( checked ) {
							var chk = input.eq(i).attr('name');
							response = jQuery( '#'+chk+'_'+(i+1) ).val();
						}
					});
				},
				...
				sort_answer: function() {
					...
					$items.each(function(i, v) {
						var $this = $(this);
						$this.removeClass('wpProQuiz_answerCorrect');
						$this.removeClass('wpProQuiz_answerIncorrect');
						var txt = $.trim($this.text());
						if(txt == data.correct[i] ) {
				...
				matrix_sort_answer: function() { // TODO
					var $items = $questionList.children();
					...
					$items.each(function(idx, val) {
						var $this = $(this);
						var $stringUl = $this.find('.wpProQuiz_maxtrixSortCriterion');
						$stringUl.removeClass('wpProQuiz_answerCorrect');
						$stringUl.removeClass('wpProQuiz_answerIncorrect');	

						var match = false;
						var stringItem = $stringUl.children()[0];
						if ( stringItem ) {
							var pos = $(stringItem).data("pos");
							match = ( data.correct[idx] == pos )
						}
						if (!match) {
							correct = false;
						}
						
						plugin.methode.marker($stringUl, match);							
					});
				},
				
				free_answer: function() {
					var $li = $questionList.children();
					var value = $.trim( $li.find('.wpProQuiz_questionInput').val() );
					if( $.inArray( value.toLowerCase(), data.correct ) >= 0) {
						$li.removeClass('wpProQuiz_answerCorrect');
						$li.removeClass('wpProQuiz_answerIncorrect');
...
//					});
		...
		plugin.methode = {
			 ...
			 */
			/*Custom Question Check handler method
			*Added by Bharat - Modified by Scitent
			*
			*Called by 'Check' button click
			*
			*Calls wdmChecker method to validate answer.
			*
			*Display response message to user.
			*
			*Date: 26 Nov, 2014
			*/
			wdmcheckQuestion: function(list, endCheck) {
				var responses = {};
				var r = {};
				...
				config.checkAnswers = {list: list, responses: responses, endCheck:endCheck};

				plugin.methode.showSpinner();
					
					// var wdmresult = wdmchecker(name, data, $this, $questionList);
					plugin.methode.ajax({
					  ...
					  data: {
						...
						list.each(function() {
							...
							if(result && result.c) { // correct
								var feedbackMssg = (!!result.e && !!result.e.AnswerMessage) ? result.e.AnswerMessage : '';
								$this.find('.wpProQuiz_incorrect').hide();
								$this.find('.wpProQuiz_correct').show(); //.append(feedbackMssg);
								$this.find('.wdmProQuiz_response').show()
									.empty().html(feedbackMssg);
								$this.find('[name="wdmcheck"]').hide();
								$this.find('[name="next"]').show();
								//Marker
								plugin.methode.markCorrectIncorrect(result, $this, $questionList);								
								// results['comp'].correctQuestions += 1;
							} else { // incorrect
								var feedbackMssg = (!!result.e && !!result.e.AnswerMessage) ? result.e.AnswerMessage : 'Please try again.';
								$this.find('.wpProQuiz_correct').hide();
								$this.find('.wdmProQuiz_response').show()
									.empty().html(feedbackMssg);
								$this.find('.wpProQuiz_incorrect').show()
									.children('span').first().addClass('wpProQuiz_answerIncorrect');
								$this.find('.wpProQuiz_questionInput').removeAttr('disabled');
							}
			...
			startQuiz: function(loadData) {
				...
				/* case 4 is custom case added to support 'Check - Correct' quiz mode
				 * Added by Bharat
				 */ 
...
//						$questionList.sortable("destroy");
			...
			initQuiz: function() {
				...
				/* Added by Bharat
				 *
				 * Calls Question Check handler method on click on check button.
				 */
				
				$e.find(globalNames.wdmcheck).click(function() {
					if(bitOptions.forcingQuestionSolve && !quizSolved[currentQuestion.index()] 
							&& (bitOptions.quizSummeryHide || !bitOptions.reviewQustion)) {
					...
					plugin.methode.wdmcheckQuestion();
				});
