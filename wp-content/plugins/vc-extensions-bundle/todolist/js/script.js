jQuery(document).ready(function($) {
	jQuery('.cqlist-container').each(function(index) {
		...
		var _clickedicon = $(this).data('clickedicon') || 'fa-check-square-o';
		...
		if(_isclickable=="yes"){
			jQuery( ".cqlist ul > li > a", _this).on('click', function(event) {
				...
				var _icondiv = $(this).find('div');
				var _icon = _icondiv.data('icon');

				if(_item.hasClass('done')){
					_item.removeClass('done');
					_icondiv.removeClass(_clickedicon).addClass(_icon);
				}else{
					_item.addClass('done');
					_icondiv.removeClass(_icon).addClass(_clickedicon);
				}
				return false;
			}).css('cursor', 'pointer');;
