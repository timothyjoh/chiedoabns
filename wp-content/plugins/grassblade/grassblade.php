...
*/
define("GRASSBLADE_VERSION", "1.4.0");
...
function grassblade($attr) {
	...
	if ($post->post_type == "sfwd-quiz") {
		error_log("I'm inside a quiz");
		$course_id = learndash_get_setting($post, "course");
	} else if ($post->post_type == "sfwd-topic") {
		error_log("I'm inside a topic");
		$lesson_id = learndash_get_setting($post, "lesson");
		$course_id = learndash_get_setting(get_post($lesson_id), "course");
	} else if ($post->post_type == "sfwd-lessons") {
		error_log("I'm inside a lesson");
		$course_id = learndash_get_setting($post, "course");
	} else {
		$course_id = $post->ID;	
	}
	error_log("Course: ".$course_id);
	$key = get_active_enrollment_key( $wp_user_id, $course_id );
	error_log("Key: ".$key);
	if ($key)
	$registration = $key;
...
class grassblade_xapi_companion {
	...
	function define_fields() {
		...
		$this->fields = array(
			...
			// LFF -- Add secondary config for Iron.io endpoint
			array( 'id' => 'iron-endpoint', 'label' => __( 'Iron.io URL', 'grassblade' ), 'placeholder' => '', 'type' => 'text', 'values'=> '', 'never_hide' => true , 'help' => __( 'URL for Iron.io instance (optional)', 'grassblade')),
			array( 'id' => 'endpoint', 'label' => __( 'LRS Endpoint URL', 'grassblade' ), 'placeholder' => '', 'type' => 'text', 'values'=> '', 'never_hide' => true , 'help' => __( 'provided by your LRS, check FAQ below for details', 'grassblade')),
			array( 'id' => 'user', 'label' => __( 'LRS API User', 'grassblade' ), 'placeholder' => '', 'type' => 'text', 'values'=> '', 'never_hide' => true ,'help' => __( 'provided by your LRS, check FAQ below for details', 'grassblade')),
			array( 'id' => 'password', 'label' => __( 'LRS API Password', 'grassblade' ),  'placeholder' => '', 'type' => 'text', 'values'=> '', 'never_hide' => true ,'help' => __( 'provided by your LRS, check FAQ below for details', 'grassblade')),
...
function grassblade_connect() {
	...
	// LFF -- Send the new Iron.io enpoint to the NSS_XAPI instance 
	$xapi = new NSS_XAPI($grassblade_settings["endpoint"], $grassblade_settings["user"], 
						 $grassblade_settings["password"], $grassblade_settings["iron-endpoint"]);
