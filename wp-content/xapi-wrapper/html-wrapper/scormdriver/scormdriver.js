...
return stmt;},sendStatement:function(stmt,callback){this.log("sendStatement");var lrs,statement,callbackWrapper,rsCount=this.recordStores.length,i,msg;if(rsCount>0){statement=this.prepareStatement(stmt);if(rsCount===1){callbackWrapper=callback;}
else{if(typeof callback==="function"){callbackWrapper=function(){this.log("sendStatement - callbackWrapper: "+rsCount);if(rsCount>1){rsCount-=1;}
else if(rsCount===1){callback.apply(this,arguments);}
else{this.log("sendStatement - unexpected record store count: "+rsCount);}};}}
for(i=0;i<rsCount;i+=1){lrs=this.recordStores[i];lrs.saveStatement(statement,{callback:callbackWrapper});}}
...
else{this.log(msg);}}},getStatement:function(stmtId,callback){this.log("getStatement");var lrs,statement,callbackWrapper,rsCount=this.recordStores.length,i,msg;if(rsCount>0){if(rsCount===1){callbackWrapper=callback;}
else{if(typeof callback==="function"){callbackWrapper=function(){this.log("sendStatement - callbackWrapper: "+rsCount);if(rsCount>1){rsCount-=1;}
else if(rsCount===1){callback.apply(this,arguments);}
else{this.log("sendStatement - unexpected record store count: "+rsCount);}};}}
for(i=0;i<rsCount;i+=1){lrs=this.recordStores[i];lrs.retrieveStatement(stmtId,callbackWrapper);}}
else{msg="[warning] getStatement: No LRSs added yet (statement not sent)";if(TinCan.environment().isBrowser){alert(this.LOG_SRC+": "+msg);}
else{this.log(msg);}}},sendStatements:function(stmts,callback){this.log("sendStatements");var lrs,statements=[],callbackWrapper,rsCount=this.recordStores.length,i,msg;if(rsCount>0){if(stmts.length>0){for(i=0;i<stmts.length;i+=1){statements.push(this.prepareStatement(stmts[i]));}
if(rsCount===1){callbackWrapper=callback;}
else{if(typeof callback==="function"){callbackWrapper=function(){this.log("sendStatements - callbackWrapper: "+rsCount);if(rsCount>1){rsCount-=1;}
else if(rsCount===1){callback.apply(this,arguments);}
else{this.log("sendStatements - unexpected record store count: "+rsCount);}};}}
for(i=0;i<rsCount;i+=1){lrs=this.recordStores[i];lrs.saveStatements(statements,{callback:callbackWrapper});}}}
...
else{this.log(msg);}}},getStatements:function(cfg){this.log("getStatements");var queryCfg={},lrs,params,msg;if(this.recordStores.length>0){lrs=this.recordStores[0];cfg=cfg||{};params=cfg.params||{};if(cfg.sendActor&&this.actor!==null){params.actor=this.actor;}
if(cfg.sendActivity&&this.activity!==null){params.activity=this.activity;}
if(typeof params.registration==="undefined"&&this.registration!==null){params.registration=this.registration;}
params.sparse=cfg.sparse||"false";queryCfg={params:params};if(typeof cfg.callback!=="undefined"){queryCfg.callback=cfg.callback;}
...
else{this.log(msg);}}};TinCan.DEBUG=false;TinCan.enableDebug=function(){TinCan.DEBUG=true;};TinCan.disableDebug=function(){TinCan.DEBUG=false;};TinCan.versions=function(){return["0.95","0.9"];};TinCan.environment=function(){if(_environment===null){_environment={};if(typeof window!=="undefined"){_environment.isBrowser=true;_environment.isIE=false;if(typeof XDomainRequest!=="undefined"){_environment.isIE=true;}}
...
return"{"+sOutput.substr(0,sOutput.length-1)+"}";}
return typeof vContent==="string"?"\""+vContent.replace(/"/g,"\\$&")+"\"":String(vContent);}};}}}());(function(){"use strict";TinCan.Utils={getUUID:function(){return'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,function(c){var r=Math.random()*16|0,v=c=='x'?r:(r&0x3|0x8);return v.toString(16);});},getISODateString:function(d){function pad(val,n){var padder,tempVal;if(val===null){val=0;}
if(n===null){n=2;}
...
+pad(d.getUTCMinutes())+':'
+pad(d.getUTCSeconds())+'.'
+pad(d.getUTCMilliseconds(),3)+'Z';},getSHA1String:function(str){return CryptoJS.SHA1(str).toString(CryptoJS.enc.Hex);},getLangDictionaryValue:function(prop,lang){var langDict=this[prop],key;if(typeof lang!=="undefined"&&typeof langDict[lang]!=="undefined"){return langDict[lang];}
...
return{path:parts[0],params:params};},getServerRoot:function(absoluteUrl){var urlParts=absoluteUrl.split("/");return urlParts[0]+"//"+urlParts[2];}};}());(function(){"use strict";var IE="ie",LRS=TinCan.LRS=function(cfg){this.log("constructor");this.endpoint=null;this.version=null;this.auth=null;this.allowFail=true;this.alertOnRequestFailure=true;this.extended=null;this._requestMode="native";this.init(cfg);};LRS.prototype={LOG_SRC:"LRS",log:TinCan.prototype.log,init:function(cfg){this.log("init");var urlParts,schemeMatches,isXD,env=TinCan.environment();cfg=cfg||{};if(!cfg.hasOwnProperty("endpoint")){if(env.isBrowser&&this.alertOnRequestFailure){alert("[error] LRS invalid: no endpoint");}
throw{code:3,mesg:"LRS invalid: no endpoint"};}
this.endpoint=cfg.endpoint;if(cfg.hasOwnProperty("allowFail")){this.allowFail=cfg.allowFail;}
if(cfg.hasOwnProperty("auth")){this.auth=cfg.auth;}
if(cfg.hasOwnProperty("extended")){this.extended=cfg.extended;}
urlParts=cfg.endpoint.toLowerCase().match(/([A-Za-z]+:)\/\/([^:\/]+):?(\d+)?(\/.*)?$/);if(env.isBrowser){schemeMatches=location.protocol.toLowerCase()===urlParts[1];isXD=(!schemeMatches||location.hostname.toLowerCase()!==urlParts[2]||location.port!==(urlParts[3]!==null?urlParts[3]:(urlParts[1]==="http:"?"80":"443")));if(isXD&&env.isIE){if(schemeMatches){this._requestMode=IE;}
else{if(cfg.allowFail){if(this.alertOnRequestFailure){alert("[warning] LRS invalid: cross domain request for differing scheme in IE");}}
else{if(this.alertOnRequestFailure){alert("[error] LRS invalid: cross domain request for differing scheme in IE");}
throw{code:2,mesg:"LRS invalid: cross domain request for differing scheme in IE"};}}}}
else{this.log("Unrecognized environment not supported: "+env);}
if(typeof cfg.version!=="undefined"){this.log("version: "+cfg.version);this.version=cfg.version;}
else{this.version=TinCan.versions()[0];}},sendRequest:function(cfg){this.log("sendRequest");var xhr,finished=false,location=window.location,fullUrl=this.endpoint+cfg.url,headers={},data,requestCompleteResult,until,prop,pairs=[],self=this;if(cfg.url.indexOf("http")===0){fullUrl=cfg.url;}
...
xhr=new XMLHttpRequest();xhr.open(cfg.method,fullUrl,(typeof cfg.callback!=="undefined"));for(prop in headers){if(headers.hasOwnProperty(prop)){xhr.setRequestHeader(prop,headers[prop]);}}
data=cfg.data;}
else if(this._requestMode===IE){this.log("sendRequest using XDomainRequest");fullUrl+="?method="+cfg.method;for(prop in cfg.params){if(cfg.params.hasOwnProperty(prop)){pairs.push(prop+"="+encodeURIComponent(cfg.params[prop]));}}
...
data=pairs.join("&");xhr=new XDomainRequest();xhr.open("POST",fullUrl);}
else{this.log("sendRequest unrecognized _requestMode: "+this._requestMode);}
function requestComplete(){self.log("requestComplete: "+finished+", xhr.status: "+xhr.status);var notFoundOk;if(!finished){finished=true;notFoundOk=(cfg.ignore404&&xhr.status===404);if(xhr.status===undefined||(xhr.status>=200&&xhr.status<400)||notFoundOk){if(cfg.callback){cfg.callback(null,xhr);}
else{requestCompleteResult={err:null,xhr:xhr};return requestCompleteResult;}}
else{if(xhr.status>0){requestCompleteResult={err:xhr.status,xhr:xhr};if(self.alertOnRequestFailure){alert("[warning] There was a problem communicating with the Learning Record Store. ("+xhr.status+" | "+xhr.responseText+")");}
if(cfg.callback){cfg.callback(xhr.status,xhr);}}
return requestCompleteResult;}}
else{return requestCompleteResult;}}
xhr.onreadystatechange=function(){if(xhr.readyState===4){requestComplete();}};xhr.onload=requestComplete;xhr.onerror=requestComplete;xhr.send(data);if(!cfg.callback){if(this._requestMode===IE){until=1000+Date.now();this.log("sendRequest - until: "+until+", finished: "+finished);while(Date.now()<until&&!finished){this.__delay();}}
...
requestCfg={url:"statements",method:"PUT",params:{statementId:stmt.id},data:JSON.stringify(stmt.asVersion(this.version))};if(typeof cfg.callback!=="undefined"){requestCfg.callback=cfg.callback;}
...
requestCfg={url:"statements",method:"GET",params:{statementId:stmtId}};if(typeof cfg.callback!=="undefined"){callbackWrapper=function(err,xhr){var result=xhr;if(err===null){result=TinCan.Statement.fromJSON(xhr.responseText);}
...
return requestResult;},saveStatements:function(stmts,cfg){this.log("saveStatements");var versionedStatements=[],requestCfg,i;if(!TinCan.environment().isBrowser){this.log("error: environment not implemented");return;}
cfg=cfg||{};if(stmts.length>0){for(i=0;i<stmts.length;i+=1){versionedStatements.push(stmts[i].asVersion(this.version));}
...
cfg=cfg||{};cfg.params=cfg.params||{};if(cfg.params.hasOwnProperty("target")){cfg.params.object=cfg.params.target;}
requestCfg=this._queryStatementsRequestCfg(cfg);if(typeof cfg.callback!=="undefined"){callbackWrapper=function(err,xhr){var result=xhr;if(err===null){result=TinCan.StatementsResult.fromJSON(xhr.responseText);}
cfg.callback(err,result);};requestCfg.callback=callbackWrapper;}
...
return requestResult;},_queryStatementsRequestCfg:function(cfg){this.log("_queryStatementsRequestCfg");var params={},returnCfg={url:"statements",method:"GET",params:params},jsonProps=["actor","object","instructor"],idProps=["verb"],valProps=["registration","context","since","until","limit","authoritative","sparse","ascending"],i;for(i=0;i<jsonProps.length;i+=1){if(typeof cfg.params[jsonProps[i]]!=="undefined"){params[jsonProps[i]]=JSON.stringify(cfg.params[jsonProps[i]].asVersion(this.version));}}
...
if(typeof cfg.lastSHA1!=="undefined"&&cfg.lastSHA1!==null){requestCfg.headers={"If-Matches":cfg.lastSHA1};}
...
if(typeof cfg.account==="object"&&cfg.account!==null&&typeof cfg.account.homePage==="undefined"){if(cfg.account.length===0){delete cfg.account;}
...
return"";},asVersion:function(version){this.log("asVersion: "+version);var result={objectType:this.objectType};version=version||TinCan.versions()[0];if(version==="0.9"){if(this.mbox!==null){result.mbox=[this.mbox];}
...
return result;}};Agent.fromJSON=function(agentJSON){Agent.prototype.log("fromJSON");var _agent=JSON.parse(agentJSON);return new Agent(_agent);};}());(function(){"use strict";var Group=TinCan.Group=function(cfg){this.log("constructor");this.member=[];this.init(cfg);};Group.prototype={objectType:"Group",LOG_SRC:'Group',log:TinCan.prototype.log,init:function(cfg){this.log("init");}};}());(function(){"use strict";var _downConvertMap={"http://adlnet.gov/expapi/verbs/experienced":"experienced","http://adlnet.gov/expapi/verbs/attended":"attended","http://adlnet.gov/expapi/verbs/attempted":"attempted","http://adlnet.gov/expapi/verbs/completed":"completed","http://adlnet.gov/expapi/verbs/passed":"passed","http://adlnet.gov/expapi/verbs/failed":"failed","http://adlnet.gov/expapi/verbs/answered":"answered","http://adlnet.gov/expapi/verbs/interacted":"interacted","http://adlnet.gov/expapi/verbs/imported":"imported","http://adlnet.gov/expapi/verbs/created":"created","http://adlnet.gov/expapi/verbs/shared":"shared","http://adlnet.gov/expapi/verbs/voided":"voided"},Verb=TinCan.Verb=function(cfg){this.log("constructor");this.id=null;this.display=null;this.init(cfg);};Verb.prototype={LOG_SRC:"Verb",log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["id","display"],prop;if(typeof cfg==="string"){for(prop in _downConvertMap){if(_downConvertMap.hasOwnProperty(prop)&&_downConvertMap[prop]===cfg){cfg=_downConvertMap[prop];}}
this.id=cfg;this.display={und:this.id};}
else{cfg=cfg||{};for(i=0;i<directProps.length;i+=1){if(cfg.hasOwnProperty(directProps[i])&&cfg[directProps[i]]!==null){this[directProps[i]]=cfg[directProps[i]];}}}},toString:function(lang){this.log("toString");if(this.display!==null){return this.getLangDictionaryValue("display",lang);}
...
return result;}};Score.fromJSON=function(scoreJSON){Score.prototype.log("fromJSON");var _score=JSON.parse(scoreJSON);return new Score(_score);};}());(function(){"use strict";var Context=TinCan.Context=function(cfg){this.log("constructor");this.registration=null;this.instructor=null;this.team=null;this.contextActivities=null;this.revision=null;this.platform=null;this.language=null;this.statement=null;this.extensions=null;this.init(cfg);};Context.prototype={LOG_SRC:'Context',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["registration","revision","platform","language","statement","extensions"],agentGroupProps=["instructor","team"],contextActivityProps=["parent","grouping","other"],prop,val;cfg=cfg||{};for(i=0;i<directProps.length;i+=1){prop=directProps[i];if(cfg.hasOwnProperty(prop)&&cfg[prop]!==null){this[prop]=cfg[prop];}}
...
this[prop]=val;}}
if(cfg.hasOwnProperty("contextActivities")&&cfg.contextActivities!==null){this.contextActivities={};for(i=0;i<contextActivityProps.length;i+=1){prop=contextActivityProps[i];if(cfg.contextActivities.hasOwnProperty(prop)&&cfg.contextActivities[prop]!==null){val=cfg.contextActivities[prop];if(!(val instanceof TinCan.Activity)){val=typeof val==='string'?{id:val}:val;val=new TinCan.Activity(val);}
this.contextActivities[prop]=val;}}}},asVersion:function(version){this.log("asVersion");var result={},optionalDirectProps=["registration","revision","platform","language","statement","extensions"],optionalObjProps=["instructor","team"],contextActivityProps=["parent","grouping","other"],i,prop;version=version||TinCan.versions()[0];for(i=0;i<optionalDirectProps.length;i+=1){if(this[optionalDirectProps[i]]!==null){result[optionalDirectProps[i]]=this[optionalDirectProps[i]];}}
for(i=0;i<optionalObjProps.length;i+=1){if(this[optionalObjProps[i]]!==null){result[optionalObjProps[i]]=this[optionalObjProps[i]].asVersion(version);}}
if(this.contextActivities!==null){result.contextActivities={};for(i=0;i<contextActivityProps.length;i+=1){prop=contextActivityProps[i];if(typeof this.contextActivities[prop]!=="undefined"&&this.contextActivities[prop]!==null){result.contextActivities[prop]=this.contextActivities[prop].asVersion(version);}}}
return result;}};Context.fromJSON=function(contextJSON){Context.prototype.log("fromJSON");var _context=JSON.parse(contextJSON);return new Context(_context);};}());(function(){"use strict";var Activity=TinCan.Activity=function(cfg){this.log("constructor");this.objectType="Activity";this.id=null;this.definition=null;this.init(cfg);};Activity.prototype={LOG_SRC:'Activity',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["id"];cfg=cfg||{};if(cfg.hasOwnProperty("definition")){this.definition=new TinCan.ActivityDefinition(cfg.definition);}
...
return"";},asVersion:function(version){this.log("asVersion");var result={id:this.id,objectType:this.objectType};version=version||TinCan.versions()[0];if(this.definition!==null){result.definition=this.definition.asVersion(version);}
return result;}};Activity.fromJSON=function(activityJSON){Activity.prototype.log("fromJSON");var _activity=JSON.parse(activityJSON);return new Activity(_activity);};}());(function(){"use strict";var InteractionComponent=TinCan.InteractionComponent=function(cfg){this.log("constructor");this.id=null;this.description=null;this.init(cfg);};InteractionComponent.prototype={LOG_SRC:'InteractionComponent',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["id","description"];cfg=cfg||{};for(i=0;i<directProps.length;i+=1){if(cfg.hasOwnProperty(directProps[i])&&cfg[directProps[i]]!==null){this[directProps[i]]=cfg[directProps[i]];}}},asVersion:function(version){this.log("asVersion");var result={id:this.id},optionalDirectProps=["description"],i,prop;version=version||TinCan.versions()[0];for(i=0;i<optionalDirectProps.length;i+=1){prop=optionalDirectProps[i];if(this[prop]!==null){result[prop]=this[prop];}}
return result;},getLangDictionaryValue:TinCan.Utils.getLangDictionaryValue};}());(function(){"use strict";var _downConvertMap={"http://adlnet.gov/expapi/activities/course":"course","http://adlnet.gov/expapi/activities/module":"module","http://adlnet.gov/expapi/activities/meeting":"meeting","http://adlnet.gov/expapi/activities/media":"media","http://adlnet.gov/expapi/activities/performance":"performance","http://adlnet.gov/expapi/activities/simulation":"simulation","http://adlnet.gov/expapi/activities/assessment":"assessment","http://adlnet.gov/expapi/activities/interaction":"interaction","http://adlnet.gov/expapi/activities/cmi.interaction":"cmi.interaction","http://adlnet.gov/expapi/activities/question":"question","http://adlnet.gov/expapi/activities/objective":"objective","http://adlnet.gov/expapi/activities/link":"link"},ActivityDefinition=TinCan.ActivityDefinition=function(cfg){this.log("constructor");this.name=null;this.description=null;this.type=null;this.extensions=null;this.interactionType=null;this.correctResponsesPattern=null;this.choices=null;this.scale=null;this.source=null;this.target=null;this.steps=null;this.init(cfg);};ActivityDefinition.prototype={LOG_SRC:'ActivityDefinition',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,j,prop,directProps=["name","description","extensions","correctResponsesPattern"],interactionComponentProps=[];cfg=cfg||{};if(cfg.hasOwnProperty("type")&&cfg.type!==null){for(prop in _downConvertMap){if(_downConvertMap.hasOwnProperty(prop)&&_downConvertMap[prop]===cfg.type){cfg.type=_downConvertMap[prop];}}
this.type=cfg.type;}
...
if(interactionComponentProps.length>0){for(i=0;i<interactionComponentProps.length;i+=1){prop=interactionComponentProps[i];if(cfg.hasOwnProperty(prop)&&cfg[prop]!==null){this[prop]=[];for(j=0;j<cfg[prop].length;j+=1){this[prop].push(new TinCan.InteractionComponent(cfg[prop][j]));}}}}}
...
return result;},getLangDictionaryValue:TinCan.Utils.getLangDictionaryValue};ActivityDefinition.fromJSON=function(definitionJSON){ActivityDefinition.prototype.log("fromJSON");var _definition=JSON.parse(definitionJSON);return new ActivityDefinition(_definition);};}());(function(){"use strict";var StatementRef=TinCan.StatementRef=function(cfg){this.log("constructor");this.id=null;this.init(cfg);};StatementRef.prototype={objectType:"StatementRef",LOG_SRC:"StatementRef",log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["id"],val;cfg=cfg||{};for(i=0;i<directProps.length;i+=1){if(cfg.hasOwnProperty(directProps[i])&&cfg[directProps[i]]!==null){this[directProps[i]]=cfg[directProps[i]];}}},toString:function(lang){this.log("toString");return this.id;},asVersion:function(version){this.log("asVersion");return{objectType:this.prototype.objectType,id:this.id};}};}());(function(){"use strict";var SubStatement=TinCan.SubStatement=function(cfg){this.log("constructor");this.actor=null;this.verb=null;this.target=null;this.result=null;this.context=null;this.timestamp=null;this.init(cfg);};SubStatement.prototype={objectType:"SubStatement",LOG_SRC:"SubStatement",log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["timestamp"],val;cfg=cfg||{};if(cfg.hasOwnProperty("object")){cfg.target=cfg.object;}
if(cfg.hasOwnProperty("actor")){this.actor=new TinCan.Agent(cfg.actor);}
if(cfg.hasOwnProperty("verb")){this.verb=new TinCan.Verb(cfg.verb);}
if(cfg.hasOwnProperty("target")){this.target=new TinCan.Activity(cfg.target);}
if(cfg.hasOwnProperty("result")){this.result=new TinCan.Result(cfg.result);}
...
(this.target!==null?this.target.toString(lang):"");},asVersion:function(version){this.log("asVersion");var result;version=version||TinCan.versions()[0];result={actor:this.actor.asVersion(version),verb:this.verb.asVersion(version),object:this.target.asVersion(version)};if(this.result!==null){result.result=this.result.asVersion(version);}
return result;}};}());(function(){"use strict";var Statement=TinCan.Statement=function(cfg,storeOriginal){this.log("constructor");this.id=null;this.actor=null;this.verb=null;this.target=null;this.result=null;this.context=null;this.timestamp=null;this.stored=null;this.authority=null;this.voided=false;this.degraded=false;this.inProgress=null;this.originalJSON=null;if(storeOriginal){this.originalJSON=JSON.stringify(cfg,null,storeOriginal);}
this.init(cfg);};Statement.prototype={LOG_SRC:'Statement',log:TinCan.prototype.log,init:function(cfg){this.log("init");var i,directProps=["id","stored","timestamp","inProgress","voided"],val;cfg=cfg||{};if(cfg.hasOwnProperty("object")){cfg.target=cfg.object;}
...
if(cfg.actor.objectType==="Agent"){this.actor=new TinCan.Agent(cfg.actor);}else if(cfg.actor.objectType==="Group"){this.actor=new TinCan.Group(cfg.actor);}}
...
if(cfg.authority.objectType==="Agent"){this.actor=new TinCan.Agent(cfg.actor);}else if(cfg.actor.objectType==="Group"){this.actor=new TinCan.Group(cfg.actor);}}
if(cfg.hasOwnProperty("verb")){this.verb=new TinCan.Verb(cfg.verb);}
if(cfg.hasOwnProperty("target")){if(typeof cfg.target.objectType==="undefined"){cfg.target.objectType="Activity";}
if(cfg.target.objectType==="Activity"){this.target=new TinCan.Activity(cfg.target);}else if(cfg.target.objectType==="Agent"){this.target=new TinCan.Agent(cfg.target);}else if(cfg.target.objectType==="SubStatement"){this.target=new TinCan.SubStatement(cfg.target);}else if(cfg.target.objectType==="StatementRef"){this.target=new TinCan.StatementRef(cfg.target);}else{this.log("Unrecognized target type: "+cfg.target.objectType);}}
if(cfg.hasOwnProperty("result")){this.result=new TinCan.Result(cfg.result);}
if(cfg.hasOwnProperty("context")){this.context=new TinCan.Context(cfg.context);}
for(i=0;i<directProps.length;i+=1){if(cfg.hasOwnProperty(directProps[i])&&cfg[directProps[i]]!==null){this[directProps[i]]=cfg[directProps[i]];}}
if(this.id===null){this.id=TinCan.Utils.getUUID();}
if(this.timestamp===null){this.timestamp=TinCan.Utils.getISODateString(new Date());}},toString:function(lang){this.log("toString");return(this.actor!==null?this.actor.toString(lang):"")+" "+
(this.verb!==null?this.verb.toString(lang):"")+" "+
(this.target!==null?this.target.toString(lang):"");},asVersion:function(version){this.log("asVersion");var result,optionalDirectProps=["id","timestamp","stored","voided"],optionalObjProps=["result","context","authority"],i;version=version||TinCan.versions()[0];result={actor:this.actor.asVersion(version),verb:this.verb.asVersion(version),object:this.target.asVersion(version)};for(i=0;i<optionalDirectProps.length;i+=1){if(this[optionalDirectProps[i]]!==null){result[optionalDirectProps[i]]=this[optionalDirectProps[i]];}}
for(i=0;i<optionalObjProps.length;i+=1){if(this[optionalObjProps[i]]!==null){result[optionalObjProps[i]]=this[optionalObjProps[i]].asVersion(version);}}
if(version==="0.9"&&this.inProgress!==null){result.inProgress=this.inProgress;}
return result;}};}());(function(){"use strict";var StatementsResult=TinCan.StatementsResult=function(cfg){this.log("constructor");this.statements=null;this.more=null;this.init(cfg);};StatementsResult.prototype={LOG_SRC:'StatementsResult',log:TinCan.prototype.log,init:function(cfg){this.log("init");cfg=cfg||{};if(cfg.hasOwnProperty("statements")){this.statements=cfg.statements;}
...
899497514);a=l;l=k;k=j<<30|j>>>2;j=g;g=c}e[0]=e[0]+g|0;e[1]=e[1]+j|0;e[2]=e[2]+k|0;e[3]=e[3]+l|0;e[4]=e[4]+a|0},_doFinalize:function(){var i=this._data,h=i.words,e=8*this._nDataBytes,g=8*i.sigBytes;h[g>>>5]|=128<<24-g%32;h[(g+64>>>9<<4)+15]=e;i.sigBytes=4*h.length;this._process()}});i.SHA1=m._createHelper(n);i.HmacSHA1=m._createHmacHelper(n)})();var TC_COURSE_ID,TC_COURSE_NAME,TC_COURSE_DESC,TC_RECORD_STORES;var TCAPI_STATUS="",TCAPI_STATUS_CHANGED=false,TCAPI_SCORE={},TCAPI_COMPLETION_STATUS="",TCAPI_SATISFACTION_STATUS=null,TCAPI_UPDATES_PENDING=false,TCAPI_IN_PROGRESS=false,TCAPI_NO_ERROR="",TCAPI_VERB_COMPLETED="http://adlnet.gov/expapi/verbs/completed",TCAPI_VERB_EXPERIENCED="http://adlnet.gov/expapi/verbs/experienced",TCAPI_VERB_ATTEMPTED="http://adlnet.gov/expapi/verbs/attempted",TCAPI_VERB_ANSWERED="http://adlnet.gov/expapi/verbs/answered",TCAPI_VERB_PASSED="http://adlnet.gov/expapi/verbs/passed",TCAPI_VERB_FAILED="http://adlnet.gov/expapi/verbs/failed",TCAPI_INIT_VERB=TCAPI_VERB_ATTEMPTED,TCAPI_INTERACTION="http://adlnet.gov/expapi/activities/cmi.interaction",TCAPI_INTERACTION_TYPE_TRUE_FALSE="true-false",TCAPI_INTERACTION_TYPE_CHOICE="choice",TCAPI_INTERACTION_TYPE_FILL_IN="fill-in",TCAPI_INTERACTION_TYPE_MATCHING="matching",TCAPI_INTERACTION_TYPE_PERFORMANCE="performance",TCAPI_INTERACTION_TYPE_SEQUENCING="sequencing",TCAPI_INTERACTION_TYPE_LIKERT="likert",TCAPI_INTERACTION_TYPE_NUMERIC="numeric",TCAPI_STATE_BOOKMARK="bookmark",TCAPI_STATE_TOTAL_TIME="cumulative_time",TCAPI_STATE_SUSPEND_DATA="suspend_data",TCAPI_ERROR_INVALID_PREFERENCE=0,TCAPI_ERROR_INVALID_TIMESPAN=1,TCAPI_FUNC_NOOP=function(){},intTCAPIError,strTCAPIErrorString,strTCAPIErrorDiagnostic;var tincan;var tcapi_cache;function TCAPI_Initialize(){WriteToDebug("In TCAPI_Initialize");tcapi_cache={totalPrevDuration:null,statementQueue:[]};tincan=new TinCan({url:location.href,recordStores:TC_RECORD_STORES,activity:{id:TC_COURSE_ID,definition:{name:TC_COURSE_NAME,description:TC_COURSE_DESC}}});if(tincan.recordStores.length===0){WriteToDebug("TCAPI_Initialize - resulted in no LRS: DATA CANNOT BE STORED");alert("[error] No LRS: DATA CANNOT BE STORED");throw{code:1,mesg:"No LRS: DATA CANNOT BE STORED"};}
...
return bookmark;}
function TCAPI_SetBookmark(value,name){WriteToDebug("In TCAPI_SetBookmark - value: "+value+", name: "+name);tincan.setState(TCAPI_STATE_BOOKMARK,value);tincan.sendStatement({verb:TCAPI_VERB_EXPERIENCED,object:{id:tincan.activity.id+"/"+value,definition:{name:{"en-US":((name!==undefined&&name!=="")?name:value)}}},context:{contextActivities:{parent:tincan.activity}}},TCAPI_FUNC_NOOP);return true;}
...
return data;}
function TCAPI_SetDataChunk(value){WriteToDebug("In TCAPI_SetDataChunk");tincan.setState(TCAPI_STATE_SUSPEND_DATA,value);return true;}
...
return true;}
function TCAPI_Finish(exitType,statusWasSet){WriteToDebug("In TCAPI_Finish - exitType: "+exitType);if(exitType===EXIT_TYPE_SUSPEND){tincan.setState(TCAPI_STATE_TOTAL_TIME,TCAPI_GetPreviouslyAccumulatedTime()+GetSessionAccumulatedTime());TCAPI_SetSuspended();}
...
tincan.setState("cmi.student_preference.audio",intPercentOfMaxVolume);}
function TCAPI_SetLanguagePreference(strLanguage){WriteToDebug("In TCAPI_SetLanguagePreference strLanguage="+strLanguage);TCAPI_ClearErrorInfo();tincan.setState("cmi.student_preference.language",strLanguage);}
...
return pref;}
function TCAPI_SetSpeedPreference(intPercentOfMax){WriteToDebug("In TCAPI_SetSpeedPreference intPercentOfMax="+intPercentOfMax);var intTCAPISpeed;TCAPI_ClearErrorInfo();intTCAPISpeed=(intPercentOfMax*2)-100;WriteToDebug("intTCAPISpeed="+intTCAPISpeed);tincan.setState("cmi.student_preference.speed",intTCAPISpeed);}
...
function TCAPI_SetTextPreference(intPreference){WriteToDebug("In TCAPI_SetTextPreference intPreference="+intPreference);TCAPI_ClearErrorInfo();tincan.setState("cmi.student_preference.text",intPreference);}
